from PyQt5.QtWidgets import (
    QWidget, QVBoxLayout, QLabel, QPushButton, QLineEdit, QTextEdit, QMessageBox, QHBoxLayout, QFrame, QGridLayout
)
...
class SearchClassroomPage(QWidget):
    ...
    def set_classroon_fields(self, classroom_data: dict):
        from Frontend.src.Admin.Classroom.upload_classroom_page import UploadClassroomPage
        classroom_id = classroom_data.get("classroom_code", "").strip()
        if not classroom_id:
            QMessageBox.warning(self, "Uyarı", "Lütfen geçerli bir derslik kodu girin.")
            return

        upload_page = UploadClassroomPage(self.parent_stack, self.user_info, classroom_id)
